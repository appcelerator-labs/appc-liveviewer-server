...
Engine.run = function run(url, platform, opts, callback) {
	...
	return async.series({
		...
		process: function (next) {
			...
			return Engine.process(downloadPath, processPath, platform, opts, function (err, res) {
...
Engine.process = function process(sourcePath, processPath, platform, opts, callback) {
	opts = opts || {};
	opts.deployType = opts.deployType || CFG.DEPLOY_TYPE;
	...
	return Engine.analyze(sourcePath, function (err, analysis) {
		...
		return async.series({
			...
			alloyCompile: function (next) {
				...
				exec(CFG.BIN_ALLOY, 'compile', processPath, '--config', 'deployType=' + opts.deployType + ',platform=' + platform, function (err) {
