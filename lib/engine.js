...
Engine.run = function run(url, platform, opts, callback) {
	...
	var projectPath = path.join(os.tmpdir(), uuid.v4());
	...
	return async.series({
		...
		prepare: function (next) {
			...
			return Engine.prepare(downloadPath, projectPath, function (err) {
...
Engine.prepare = function prepare(downloadPath, projectPath, callback) {
	var isAlloy = false;
				...
				if (isFile) {
					...
					return async.detect(tests, function (test, detected) {
						...
						return fs.stat(test.input, function (err, stats) {
							return detected(!err && stats.isFile());
						});
