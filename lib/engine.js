...
exports.run = function run(url, platform, callback) {
	...
	async.series({
		...
		cpProject: function (next) {
			exec('cp', '--recursive', CFG.PATH_PROJECT, paths.tmp, next);
		},
		mvDownload: function (next) {
			exec('mv', paths.download + '/', paths.project + pathPrefix + '/', next);
		},
		alloyCompile: function (next) {
			...
			exec(CFG.BIN_ALLOY, 'compile', paths.project, '--config', 'platform=' + platform, next);
		},
		...
		rmProject: function (next) {
			exec('rm', '-rf', paths.tmp, next);
		}
	}, function (err) {

		if (err) {
			return exec('rm', '-rf', paths.zip, function (rmErr) {
				callback(rmErr || err);
			});
		}
